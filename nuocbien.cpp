...
void read ()
{
	for (int i=1; i<=n; i++)
	{
		...
		{
			cin>>seawater[i][j];
			if (seawater[i][j]>Hmax) Hmax=seawater[i][j]; // tim gia tri lon nhat trong ma tran
		}
	}
}
 
int check[102][102]; // ma tran kiem tra
int check_connect[102][102];  // ma tran kiem tra luc lien thong
void init () // khoi tao
{
	for (int i=1; i<=n; i++)
	{
